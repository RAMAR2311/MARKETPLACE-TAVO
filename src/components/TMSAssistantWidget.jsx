...
export default function TMSAssistantWidget({ theme, isOpen, onClose, onSelectProduct, products }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 max-w-sm w-full rounded-3xl shadow-2xl border-2 border-[#C59F60] overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
      
      {/* Header */}
      <div className="bg-[#0A0A0A] text-white p-4 flex items-center justify-between border-b border-[#222222]">
        <div className="flex items-center gap-3">
          <TavoIsotype className="w-8 h-8" />
          <div className="text-left">
            <h3 className="text-sm font-black font-heading flex items-center gap-1">
              <span>Asesor</span>
              <span className="text-[#C59F60]">TAVO</span>
            </h3>
            <p className="text-[10px] text-neutral-400 font-mono">Asesoría de Compra Inteligente</p>
          </div>
        </div>
      ...
      {/* Chat Body */}
      <div className={`p-4 space-y-4 text-xs text-left min-h-[300px] max-h-[400px] overflow-y-auto transition-colors ${
        isDark ? 'bg-[#111111] text-white' : 'bg-[#F9F9F9] text-[#1A1A1A]'
      }`}>
        
        {/* Welcome Message */}
        <div className="flex gap-2.5">
          <div className="w-8 h-8 rounded-full bg-[#C59F60] text-[#0A0A0A] flex items-center justify-center shrink-0 shadow-sm font-black">
            <Bot className="w-4 h-4" />
          </div>
          <div className={`p-3 rounded-2xl border shadow-sm max-w-[85%] ${
        ...
        {useCase && !budget && (
          <>
            <div className="flex gap-2.5">
              <div className="w-8 h-8 rounded-full bg-[#C59F60] text-[#0A0A0A] flex items-center justify-center shrink-0">
                <Bot className="w-4 h-4" />
              </div>
              <div className={`p-3 rounded-2xl border text-left ${
                ...
                <p className="text-neutral-300">
                  Excelente elección. ¿En qué rango de presupuesto te gustaría ubicarte?
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              >
                <span>Flagship Ultra (&gt; $4.500.000 COP)</span>
              ...
              >
                <span>Gama Alta ($1.500.000 - $4.499.000 COP)</span>
                <ChevronRight className="w-4 h-4 text-[#C59F60]" />
              </button>
            </div>
          </>
          ...
          <div className="space-y-3 pt-2">
            <div className="flex gap-2.5">
              ...
              <div className={`p-3 rounded-2xl border text-left ${
                ...
                <p className="font-extrabold text-[#C59F60]">¡Tu Selección Exclusiva TAVO VIP!</p>
                <p className="text-neutral-400 mt-1">
                  Basado en tu perfil, este es el equipo con mejor desempeño:
                </p>
              </div>
            </div>

            {/* Recommended Product Box */}
            <div className={`p-3 rounded-2xl border shadow-md flex items-center gap-3 ${
              isDark ? 'bg-[#161616] border-[#222222]' : 'bg-white border-neutral-200'
            }`}>
              <img src={recommendation.image} alt={recommendation.name} className="w-14 h-14 object-contain" />
              <div className="flex-1 text-left">
                <p className="text-[10px] font-black text-[#C59F60] uppercase font-mono">{recommendation.brand}</p>
                <h4 className={`text-xs font-black line-clamp-1 font-heading ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>
                  {recommendation.name}
                </h4>
                <p className="text-sm font-black text-[#C59F60] font-mono">${recommendation.price.toLocaleString('es-CO')} COP</p>
              </div>
            </div>
            ...
            <a
              ...
              className="w-full btn-whatsapp py-2.5 rounded-xl text-xs font-black shadow-md"
            >
            ...
            <button
              onClick={resetChat}
              className="w-full text-center text-xs text-neutral-500 hover:text-white py-1 font-mono"
            >
              ← Comenzar de nuevo
            </button>
          </div>
