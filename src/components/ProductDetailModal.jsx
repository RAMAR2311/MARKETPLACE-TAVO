...
export default function ProductDetailModal({ theme, product, onClose, onAddToCart, onToggleWishlist, isWishlisted }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className={`rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative border transition-all ${
        ...
        <button
          onClick={onClose}
          className={`absolute top-4 right-4 z-20 w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
            isDark 
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
          ...
          <div className={`md:col-span-5 flex flex-col justify-between items-center p-8 relative border-b md:border-b-0 md:border-r ${
            isDark ? 'bg-[#161616] border-[#222222]' : 'bg-[#FAFAFA] border-[#EAEAEA]'
          }`}>
            
            {/* VIP Status Badge */}
            <div className="absolute top-4 left-4 bg-[#C59F60] text-[#0A0A0A] text-[10px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-md font-mono">
              <Crown className="w-3 h-3" />
              <span>D'TAVO OFFICIAL</span>
            </div>
            ...
            <div className="py-8 flex justify-center items-center w-full">
              <img
                src={product.image}
                alt={product.name}
                className="max-h-72 object-contain drop-shadow-2xl hover:scale-105 transition-transform duration-500"
              />
            </div>

            {/* Color Selection */}
            {product.colors && product.colors.length > 0 && (
              <div className={`w-full pt-4 border-t text-left ${isDark ? 'border-[#222222]' : 'border-[#DDDDDD]'}`}>
                <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block mb-2 font-mono">
                  Color Seleccionado: <strong className="text-[#C59F60]">{selectedColor}</strong>
                </span>
          ...
          <div className="md:col-span-7 p-6 sm:p-8 text-left space-y-5">
            
            {/* Brand & Title */}
            <div>
              <span className="text-[11px] font-black text-[#C59F60] uppercase tracking-[0.2em] font-mono">
                {product.brand}
              </span>
              <h2 className={`text-2xl sm:text-3xl font-black font-heading mt-1 leading-tight ${
              ...
              <div className="flex items-center gap-2.5 mt-2">
                ...
                <span className="text-[11px] text-neutral-500 font-mono">({product.reviewsCount} reseñas verificadas)</span>
              </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border flex items-center justify-between ${
              ...
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-black text-[#C59F60] font-mono">${product.price.toLocaleString('es-CO')} COP</span>
                  {product.originalPrice > product.price && (
                    <span className="text-sm text-neutral-500 line-through font-mono">${product.originalPrice.toLocaleString('es-CO')}</span>
                  )}
                </div>
              </div>
              {product.discountPercent > 0 && (
                <div className="bg-[#C59F60] text-[#0A0A0A] font-black text-xs px-3 py-1.5 rounded-lg shadow-md font-mono">
                  Ahorras ${(product.originalPrice - product.price).toLocaleString('es-CO')} (-{product.discountPercent}%)
                </div>
              )}
            </div>
            ...
            {product.storageOptions && (
              <div>
                <label className="block text-[10px] font-bold text-neutral-400 uppercase tracking-[0.2em] mb-2 font-mono">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.storageOptions.map((opt) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-extrabold font-mono transition-all duration-200 border ${
                        selectedStorage === opt
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`pb-2.5 px-3 text-xs font-bold transition-all border-b-2 font-mono ${
                      activeTab === tab.id 
              ...
              {activeTab === 'specs' && (
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {Object.entries(product.specs).map(([key, value]) => (
                    ...
                    >
                      <span className="text-neutral-400 font-mono block uppercase text-[10px] tracking-wider">
                        {key === 'os' ? 'Sistema Operativo' : key === 'screen' ? 'Pantalla' : key === 'processor' ? 'Procesador' : key === 'camera' ? 'Cámaras' : key === 'battery' ? 'Batería' : key}:
                      </span>
                      <span className={`font-bold font-mono leading-snug block mt-0.5 ${isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>
                        {value}
                      </span>
                    </div>
                  ))}
                  ...
                  {zipCalculated && (
                    <div className="bg-[#2ECC71]/10 border border-[#2ECC71]/30 text-[#2ECC71] p-3 rounded-xl text-xs flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-neutral-300">Entrega garantizada en tu domicilio antes de las 18:00 hrs del día siguiente.</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className={`pt-4 border-t flex gap-3 ${isDark ? 'border-[#222222]' : 'border-[#EAEAEA]'}`}>
              {onToggleWishlist && (
                <button
                  type="button"
                  onClick={() => onToggleWishlist(product.id)}
                  className={`px-4 py-3.5 rounded-xl border flex items-center justify-center transition-all ${
                    isWishlisted
              ...
              <button
                ...
                className="flex-1 btn-gold-primary py-3.5 rounded-xl text-sm font-black flex items-center justify-center gap-2"
              >
                <ShoppingCart className="w-4 h-4" />
                <span>Agregar a la Cesta de Compra</span>
              </button>
            </div>
